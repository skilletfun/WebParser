from parsers.basic_functions import find_images, rebuild_redownload_images, download_images, get_response, find_element, archivate, prepare_save_folder
...
def parse(url, timeout, save_folder, redownload_numbers, do_archive, chapter_count):
    ...
    while True:
        ...
        title, images = find_images(src, 'div', 'id', '_imageList')
        ...
        if chapter_count > 0:
            res = find_element(src, 'a', 'class', '_nextEpisode')
            if not res is None:
                url = res.get('href')
